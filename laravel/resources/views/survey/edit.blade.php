            ...
            <section class="content-header">
                <div class="pull-right">
                    <form method="POST" action="{{ url('/create/'.$survey->id) }}" class="publish-form">
                        {{ csrf_field() }}
                        {{ method_field('PATCH') }}
                        <button type="button" id="publish-survey" class="btn btn-lg btn-primary"><i class="fa fa-btn fa-share"></i>Publish Template</button>
                    </form>
                </div>
            </section>
        @endif
            ...
            <section class="content-header">
                <div class="pull-right">
                    <form method="POST" action="{{ url('/create/'.$survey->id) }}" class="publish-form">
                        {{ csrf_field() }}
                        {{ method_field('PATCH') }}
                        <button type="button" id="publish-survey" class="btn btn-lg btn-primary"> <i class="fa fa-btn fa-share"></i>Publish Survey</button>
                    </form>
                </div>
            </section>
        @endif
...
@endif
    ...
    @foreach($survey->pages()->orderBy('page_no')->get() as $page)
        <div id="page{{ $page->id }}" class="page-row" data-page-number="{{ $page->page_no }}" data-id="{{ $page->id }}" data-title="{{ $page->page_title }}" data-description="{{ $page->page_description }}">
            <h4 class="survey-page-header">
                ...
                @if(!$survey->published)
                    <div class="btn-group">
                        <button type="button" class="btn btn-facebook btn-sm dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            <i class="fa fa-btn fa-wrench"></i>Actions
                            <span class="caret"></span>
                        </button>
                        <ul class="dropdown-menu page-actions">
                            <li><a data-toggle="modal" data-target="#move-copy-modal" data-action="move_page" class="move-page editable"><i class="fa fa-btn fa-cut"></i>Move Page</a></li>
                            <li><a data-toggle="modal" data-target="#move-copy-modal" data-action="copy_page" class="replicate-page editable"><i class="fa fa-btn fa-copy"></i>Replicate Page</a></li>
                            <li><a class="delete-page editable"><i class="fa fa-btn fa-eraser"></i>Delete Page</a></li>
                        </ul>
                    </div>
                @endif
            </h4>
            <div class="panel panel-primary">
                ...
                <div class="panel-footer edit-page-title editable" data-toggle="modal" data-target="#page-title-modal" data-identifier="title">
                ...
                <div class="panel-body">
                    <div class="page-description edit-page-description editable {{ $survey->published ? '' : 'hoverable' }}" data-toggle="modal" data-target="#page-title-modal" data-identifier="description">
                    ...
                    <div class="question-container">
                        ...
                        @foreach($page->questions()->orderBy('order_no')->get() as $question)
                            <div class="question-row-container">
                                ...
                                <div id="question{{ $question->id }}" class="row question-row" data-max-rating="{{ $question->option == null ? 3 : $question->option->max_rating }}" data-question-id="{{ $question->id }}" data-question-number="{{ $questionNo }}" data-question-type="{{ $question->questionType->id }}" data-is-mandatory="{{ $question->is_mandatory }}" data-has-choices="{{ $question->questionType->has_choices }}">
                                            ...
                                            @if($question->questionType->type == "Multiple Choice")
                                                @foreach($question->choices()->get() as $choice)
                                                    <div class="radio choice-row" data-choice-id="{{ $choice->id }}">
                                                        <label class="choice-label"><input type="radio" name="{{ $question->id }}"> {{ $choice->label }}</label>
                                                    </div>
                                                @endforeach
                    ...
                    @if(!$survey->published)
                        <div class="row" style="margin-top: 15px">
                            <div class="btn-group col-xs-offset-4">
                                <button type="button" class="btn btn-primary add-question" data-question-number="{{ $questionNo }}" data-page="{{ $page->page_no }}"  data-toggle="modal" data-target="#add-question-modal">
                                ...
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu">
                                        @foreach($question_types as $question_type)
                                            <li class="type-option editable"><a data-toggle="modal" data-target="#add-question-modal" data-question-number="{{ $questionNo }}" data-question-type="{{ $question_type->id }}" has-choices="{{ $question_type->has_choices }}">{{ $question_type->type }}</a></li>
                                        @endforeach
